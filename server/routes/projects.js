...
router.get('/', authMiddleware, async (req, res) => {
  try {
    ...
    const [projects, total] = await Promise.all([
      Project.find(filter).populate('owner', 'fullName email').populate('members', 'fullName email')
...
router.get('/:id/tasks', authMiddleware, requireMember, async (req, res) => {
  try {
    const tasks = await Task.find({ project: req.params.id })
      .populate('assignedTo', 'fullName email').sort({ createdAt: -1 });
    res.json(tasks);
  } catch (err) { res.status(500).json({ message: err.message }); }
});
...
// POST membre — notification added_to_project (F10)
router.post('/:id/members', authMiddleware, requireOwner, async (req, res) => {
  try {
    ...
    await logActivity('member_added', project._id, req.user.userId, {
      memberName: userToAdd.fullName, memberEmail: userToAdd.email
    });

    // Notifier le nouveau membre (F10)
...
router.delete('/:id/members/:memberId', authMiddleware, requireOwner, async (req, res) => {
  try {
    ...
    await project.save();
    await Task.updateMany({ project: project._id, assignedTo: removedId }, { $set: { assignedTo: null } });
    await logActivity('member_removed', project._id, req.user.userId, {
      memberName: removedUser?.fullName, memberEmail: removedUser?.email
    });
