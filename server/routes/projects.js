...
// POST /api/projects — réservé à tout utilisateur connecté
...
// PUT /api/projects/:id — owner seulement (F8)
...
// DELETE /api/projects/:id — owner seulement (F8)
router.delete('/:id', authMiddleware, requireOwner, async (req, res) => {
  try {
    await req.project.deleteOne();
    res.json({ message: 'Projet et tâches supprimés' });
  } catch (err) { res.status(500).json({ message: err.message }); }
});

// ---- Gestion des membres (F8) ----
...
// POST inviter un membre — owner seulement (F8)
router.post('/:id/members', authMiddleware, requireOwner, async (req, res) => {
  try {
    ...
    if (userToAdd._id.toString() === req.user.userId)
      return res.status(400).json({ message: 'Vous êtes déjà le créateur' });
    const project = req.project;
...
// DELETE retirer un membre — owner seulement (F8)
